package com.whut.yinyuepiaoliu.alimessage;
...
public class SendSMS {
    ...
    private static final String accessKeyId = "LTAItwCJ7GVEOYsJ";
    private static final String accessKeySecret = "fv0jAxVPUKfKdgSfeYHj32BXvvCHMv";

    String phoneCache = null;

    private SendSmsResponse sendSms(String phone, String code) throws ClientException {
        phoneCache = phone;
        System.out.println("将向用户" + phone + "发送验证码" + code);
        ...
        //必填:短信签名-可在短信控制台中找到
        request.setSignName("音乐漂流");
        //必填:短信模板-可在短信控制台中找到
        request.setTemplateCode("SMS_119087164");
    ...
    private QuerySendDetailsResponse querySendDetails(String bizId) throws ClientException {
        ...
        //必填-号码
        request.setPhoneNumber(phoneCache);
        //可选-流水号
    ...
    @Test
    public void sendMessageTest() throws ClientException, InterruptedException {

        //发短信
        SendSmsResponse response = sendSms("18202715298","1234");
    ...
    // 生成4位数字验证码
    private String RandomCodeDemo() {
        StringBuilder sb = new StringBuilder(4);
        for (int i = 0; i < 4; i++) {
            int ch;
            if (i == 0) {
                ch = new Random().nextInt(9) + 1;
            } else {
                ch = new Random().nextInt(10);
            }
            sb.append(ch);
        }
        return sb.toString();
